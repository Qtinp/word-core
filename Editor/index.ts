...
const getjson = (list: string, name: string) => {
  return api.command.getjson(wordDir, list, name);
};
...
const update = (list: string, name: string, file: object) => {
  return api.command.update(wordDir, list, name, file);
};
...
export default class {
  ...
  constructor(dir: string) {
    ...
    try {
      fs.mkdirSync(path.join(dir, 'word/wordConfig'));
    } catch (err) {}
    try {
      fs.mkdirSync(path.join(dir, 'word/recycleBin'));
    } catch (err) {}
   ...
   */
  getCacheWord() {
    return api.wordCommand.getCacheWord(wordDir);
  }
  ...
  del(q: string, index: string, id: string) {
    ...
    if (!word.main[q]) {
      return ' [词库核心] 当前词库不存在您要删除的问答';
    }

    if (index === 'all') {
      delete word.main[q];
    } else if (/\d+/.test(index)) {
      if (Number(index) > word.main[q].length) {
        return ' [词库核心] 您输入的序号不存在';
      }
      word.main[q].splice(Number(index) - 1, 1);
      if (word.main[q].length <= 0) {
        delete word.main[q];
      }
    } else {
  ...
  passiveList(dbName: string) {
    if (this.wordObj.wordList.indexOf(dbName) === -1) {
      return ' [词库核心] 此词库并不存在！';
    }
  ...
  killList(dbName: string) {
    ...
    try {
      fs.renameSync(
        path.join(wordDir, `word/wordList/${dbName}.json`),
        path.join(wordDir, `word/recycleBin/${dbName}.json`),
      );
      this.wordObj = this.getCacheWord();
  ...
  recoveryList(dbName: string) {
    if (this.wordObj.recycleBinList.indexOf(dbName) === -1) {
      return ' [词库核心] 回收站不存在此词库';
    }
    try {
      fs.renameSync(
        path.join(wordDir, `word/recycleBin/${dbName}.json`),
  ...
  addWriter(tid: string, mid: string) {
    ...
    if (pointer === '默认') {
      return ' [词库核心] 此词库为公共词库，无法修改';
    }
    if (!word.author) {
      return ' [词库核心] 此词库为空词库';
    }
    if (author[0] !== mid) {
      return '[词库核心] 此词库主作者并非是您';
    }
  ...
  whenOff(q: string, index: string, id: string) {
    ...
    if (index === 'all') {
      ...
      if (num > word.initiative[q].length) {
