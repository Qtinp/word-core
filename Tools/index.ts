...
export const command = {
  ...
  update: (
    ...
    try {
      fs.writeFileSync(
        path.join(dir, `./word/${list}/${name}.json`),
...
export const wordCommand = {
  ...
  getCacheWord: (dir: string) => {
    ...
    for (const a of wordListArr) {
      ...
      const initiativeKey = !data.initiative
        ? []
        : Object.keys(data.initiative);

      if (!CacheObj.wordList) {
        CacheObj.wordList = [];
      } // 将本词库名放入缓存
      CacheObj.wordList.push(name);

      for (const item of keysArr) {
        if (!CacheObj.passive[item]) {
          CacheObj.passive[item] = [];
        }
        CacheObj.passive[item].push(name); // 将此触发词所在的词库放入缓存

        if (CacheObj.keys.indexOf(item) < 0) {
          CacheObj.keys.push(item);
        }
      }

      for (const item2 of initiativeKey) {
        if (!CacheObj.initiative[item2]) {
          CacheObj.initiative[item2] = [];
        }
        CacheObj.initiative[item2] = CacheObj.initiative[item2].concat(
          data.initiative[item2],
        );
      }
    }
    return CacheObj;
